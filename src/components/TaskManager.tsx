...
interface Task {
  ...
  assignedBy?: { id: string; name: string; email: string; role: string } | null;
}
...
const TaskManager: React.FC<TaskManagerProps> = ({ mode, onCreateNewTask }) => {
  ...
  const pageTitle = isManager ? "📥 Task History (Assigned Tasks)" : "📥 My Assigned Tasks";
  ...
  const formatDate = (dateString: string, showUrgency: boolean = false) => {
    try {
      ...
      // Enhanced formatting for assignee view
  ...
  const getTaskUrgency = (deadline: string, priority: string) => {
    if (isManager) return "normal"; // No urgency for manager view
  ...
  const getTaskBorderClass = (deadline: string, priority: string) => {
    if (isManager) return ""; // No borders for manager view
  ...
  // Filter tasks based on status (only for assignee view)
  const filteredTasks = !isManager && statusFilter !== "All" 
    ? tasks.filter(task => task.status === statusFilter)
    : tasks;

  // Group tasks by urgency for assignee summary
  ...
  const renderHeader = () => (
    <div className="flex justify-between items-center mb-6">
      ...
      {isManager ? (
        <button onClick={onCreateNewTask} className="btn-primary">
          + Create New Task
        </button>
      ) : (
  ...
  const renderAssignmentInfo = (task: Task) => {
    ...
    if (!isManager && task.assignedBy) {
      return (
        <p className="mt-3 text-sm detail-item">
          <strong>👤 Assigned By:</strong> {task.assignedBy.name} ({task.assignedBy.role}) - {task.assignedBy.email}
        </p>
      );
    }
      ...
      <div>
        ...
        <div className="text-center py-8">
          <p className="text-gray-600">
            {statusFilter === "All" || isManager 
              ? "No tasks found." 
              : `No ${statusFilter.toLowerCase()} tasks.`}
          </p>
          {isManager && onCreateNewTask && (
            <button onClick={onCreateNewTask} className="btn-primary mt-4">
    ...
    <div>
      ...
      <div ref={taskListRef} className="space-y-4">
        {filteredTasks.map((task) => (
          <div
            key={task.id}
            className={`bg-white dark:bg-gray-900 rounded-2xl shadow transition overflow-hidden ${getTaskBorderClass(task.deadline, task.priority)}`}
          >
            <div
            ...
            >
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {task.company_name} • Deadline: {formatDate(task.deadline, !isManager)}
                </p>
                {!isManager && (
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`${!isManager ? 'px-3 py-1 rounded-full' : ''} text-xs font-medium ${getStatusColor(task.status, !isManager)}`}>
                  {task.status || "Pending"}
                </span>
              </div>
            </div>
            ...
            >
              <div className="py-4 border-t border-gray-200 dark:border-gray-700">
                ...
                {task.notes && (
                  <p className="mt-3 text-sm detail-item">
                    <strong>📝 Notes{!isManager ? ' from Manager' : ''}:</strong> {task.notes}
                  </p>
                )}
