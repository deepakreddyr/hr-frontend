...
const History = () => {
  ...
  const fetchSearches = async () => {
    try {
      const res = await fetch(`${REACT_APP_API_URL}/api/searches`, {
        credentials: 'include',
      });
  ...
  const handleRowClick = async (searchId: number) => {
    try {
      const res = await fetch(`${REACT_APP_API_URL}/api/search-status/${searchId}`, {
        credentials: 'include',
      });
      const data = await res.json();
  ...
  const handleCreateNewSearch = async () => {
    try {
      const res = await fetch(`${REACT_APP_API_URL}/api/create-search`, {
        method: 'POST',
        credentials: 'include',
      });
      const data = await res.json();
