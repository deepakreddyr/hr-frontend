...
const Results = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/results?searchID=${searchId}`, {
  ...
  const handleLikeToggle = async (candidateId: number, currentLiked: boolean) => {
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/like-candidate`, {
        withCredentials: true,
  ...
  const handleAddToFinalSelects = async () => {
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}/add-final-select`, {
        withCredentials: true,
  ...
  try {
    const res = await axios.post(`${process.env.REACT_APP_API_URL}/call-single`, {
      name: candidate.name,
...
const handleCallSelectedCandidates = async () => {
  try {
    ...
    const res = await axios.post(`${process.env.REACT_APP_API_URL}/call`, {
      candidates: payload
    }, { withCredentials: true });
