...
const SavedProfiles = () => {
    ...
    const fetchSavedCandidates = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/get-liked-candiates`,
          {
            headers: {
  ...
  const handleDelete = async (candidateId: number) => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/unlike-candidate`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('access_token')}`,
  ...
  const SavedProfileSkeleton = () => (
    <div className="bg-card border border-border rounded-lg p-6 animate-pulse">
      <div className="absolute top-4 right-4 w-5 h-5 bg-muted rounded-full" />
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Your favorite candidates all in one place
          </p>
        </div>
